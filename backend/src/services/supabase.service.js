...
/** Only posts that are live on LinkedIn (posted=true, status=posted, linkedin_post_id set). Used by reply job and n8n. */
export async function getPostedPosts(userId) {
  try {
    ...
    const { data, error } = await supabase
      .from('posts')
      .select('id, linkedin_post_id, activity_urn, content, hook, posted_at')
      .eq('user_id', userId)
      .eq('posted', true)
      .eq('status', 'posted')
...
export async function logEngagement(userId, data) {
  try {
    ...
    if (data.author_urn != null) row.author_urn = data.author_urn;
    if (data.author_name != null) row.author_name = data.author_name;
    if (data.author_headline != null) row.author_headline = data.author_headline;
    if (data.author_profile_url != null) row.author_profile_url = data.author_profile_url;
    if (data.permalink != null) row.permalink = data.permalink;
    if (data.media_type != null) row.media_type = data.media_type;
    if (data.original_activity_id != null) row.original_activity_id = data.original_activity_id;
    if (data.was_reshare != null) row.was_reshare = data.was_reshare;
    if (data.liked != null) row.liked = data.liked;
    if (data.commented != null) row.commented = data.commented;
    if (data.description != null) row.description = data.description;
    await supabase.from('engagement_logs').insert(row);
  } catch (e) {
...
export async function logReply(userId, data) {
  try {
    ...
    const row = {
      ...
      post_uri: data.post_uri ?? null,
      comment_urn: data.comment_urn ?? null,
      comment_text: data.comment_text ?? null,
      reply_text: data.reply_text ?? null,
      status: data.status ?? 'completed',
    };
    await supabase.from('comment_replies').insert(row);
    await supabase.from('reply_logs').insert(row);
  } catch (e) {
...
export async function getUsersWithAutoLikeCommentEnabled() {
  try {
    ...
    const { data: settingsRows, error: setErr } = await supabase
      .from('engagement_settings')
      .select('user_id, auto_liking, auto_commenting, engagement_interval_minutes, webhook_last_sent_at, comment_prompt, max_engagements_per_day, active_days, active_start_time, active_end_time');
    if (setErr) return [];
...
/**
 * Get users with auto_replying = true who have an active linkedin_connection.
 * We only return users that have a linkedin_connections row so n8n "Get LinkedIn Connection2" won't fail.
 */
export async function getUsersWithAutoReplyEnabled() {
  try {
    const supabase = getClient();
    const { data: conns, error: connErr } = await supabase
      .from('linkedin_connections')
      .select('user_id, person_urn')
      .eq('is_active', true);
    if (connErr || !conns?.length) return [];

    const { data: settingsRows, error: setErr } = await supabase
      .from('engagement_settings')
      .select('user_id, auto_replying, comment_prompt, active_days, active_start_time, active_end_time, reply_interval_minutes, reply_webhook_last_sent_at');
    if (setErr) return [];
...
/** Update reply_webhook_last_sent_at for a user (after sending to reply-to-comments webhook). */
export async function updateReplyWebhookLastSent(userId) {
  try {
    const supabase = getClient();
    const { error } = await supabase
      .from('engagement_settings')
      .update({ reply_webhook_last_sent_at: new Date().toISOString() })
      .eq('user_id', userId);
    return !error;
  } catch (e) {
    console.error(JSON.stringify({ timestamp: new Date().toISOString(), service: 'supabase', action: 'updateReplyWebhookLastSent', userId, error: e.message }));
    return false;
  }
}
