...
const LINKEDIN_REST_BASE_URL = process.env.LINKEDIN_REST_BASE_URL || 'https://api.linkedin.com';
const LINKEDIN_VOYAGER_BASE_URL = process.env.LINKEDIN_VOYAGER_BASE_URL || 'https://www.linkedin.com';
...
async function uploadImageAsset(credentials, mediaUrl) {
  ...
  const initRes = await axios.post(
    `${LINKEDIN_REST_BASE_URL}/rest/images?action=initializeUpload`,
...
async function uploadVideoAsset(credentials, videoUrl) {
  ...
  try {
    initRes = await axios.post(
      `${LINKEDIN_REST_BASE_URL}/rest/videos?action=initializeUpload`,
      {
  ...
  const finalizeRes = await axios.post(
    `${LINKEDIN_REST_BASE_URL}/rest/videos?action=finalizeUpload`,
    {
...
export async function postToLinkedIn(credentials, { hook, content, hashtags, mediaUrl, videoUrl }) {
  ...
  try {
    const res = await axios.post(`${LINKEDIN_REST_BASE_URL}/rest/posts`, body, { headers: postHeaders });
...
export async function commentOnPost(credentials, postUrn, commentText) {
  ...
  try {
    const res = await axios.post(
      `${LINKEDIN_REST_BASE_URL}/v2/socialActions/${encodeURIComponent(urn)}/comments`,
      {
...
export async function likePost(credentials, activityId) {
  ...
  try {
    const res = await axios.post(
      `${LINKEDIN_REST_BASE_URL}/v2/socialActions/${encodeURIComponent(urn)}/likes`,
      { actor: credentials.personUrn },
      {
        headers: {
          'Content-Type': 'application/json',
...
export async function getPostComments(credentials, postUrn) {
  ...
  try {
    ...
    const postRes = await axios.get(
      `${LINKEDIN_VOYAGER_BASE_URL}/voyager/api/feed/updates/${encodeURIComponent(voyagerUrn)}`,
      { headers }
    );
    ...
    const commentsRes = await axios.get(
      `${LINKEDIN_VOYAGER_BASE_URL}/voyager/api/feed/updates/${encodeURIComponent(activityUrn)}/socialDetail`,
      {
...
export async function replyToComment(credentials, activityUrn, commentUrn, replyText) {
  ...
  try {
    const res = await axios.post(
      `${LINKEDIN_REST_BASE_URL}/v2/socialActions/${encodeURIComponent(activityUrn)}/comments`,
      body,
      { headers }
    );
    console.log(JSON.stringify({
