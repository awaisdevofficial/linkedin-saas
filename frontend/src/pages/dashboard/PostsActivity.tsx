...
const PostsActivity = () => {
  ...
  const [publishChoicePost, setPublishChoicePost] = useState<Post | null>(null);
  const [publishChoiceFromApprove, setPublishChoiceFromApprove] = useState(false);
  ...
  useEffect(() => {
    ...
    const fetchPosts = async () => {
      const [postsRes, profileRes] = await Promise.all([
        client
          .from('posts')
          .select('id, hook, content, hashtags, status, posted, media_url, video_url, publish_with_video, created_at, scheduled_at, posted_at')
          .eq('user_id', user.id)
  ...
  const handlePublishNow = async (postId: string, contentOnly?: boolean) => {
    if (!accessToken) return;
    setActionLoading(`publish:${postId}`);
    try {
      await apiCalls.publishNow(accessToken, postId, contentOnly);
      const now = new Date().toISOString();
      setPosts((prev) =>
        prev.map((p) =>
          p.id === postId ? { ...p, posted: true, status: 'posted', posted_at: now } : p
        )
      );
      if (viewPost?.id === postId) {
        setViewPost((prev) => (prev ? { ...prev, posted: true, status: 'posted', posted_at: now } : null));
      }
      if (approvePost?.id === postId) {
        setApprovePost(null);
        setIsApproveDialogOpen(false);
      }
      setPublishChoicePost(null);
      setPublishChoiceFromApprove(false);
  ...
  const openPublishNow = (post: Post, fromApprove = false) => {
    const hasMedia = !!(post.media_url || post.video_url);
    if (hasMedia) {
      setPublishChoicePost(post);
      setPublishChoiceFromApprove(fromApprove);
    } else {
      if (fromApprove && approvePost) {
        handleApproveAndPublishNow(false);
      } else {
        handlePublishNow(post.id, false);
      }
    }
  };

  const handlePublishChoice = async (contentOnly: boolean) => {
    if (!publishChoicePost || !accessToken) return;
    if (publishChoiceFromApprove && approvePost?.id === publishChoicePost.id && supabase) {
      try {
        await supabase.from('posts').update({ status: 'approved' }).eq('id', publishChoicePost.id);
      } catch (_) {}
    }
    await handlePublishNow(publishChoicePost.id, contentOnly);
    setPublishChoicePost(null);
    setPublishChoiceFromApprove(false);
  };
  ...
  const handleApproveAndPublishNow = async (contentOnly?: boolean) => {
    ...
    try {
      ...
      await apiCalls.publishNow(accessToken, approvePost.id, contentOnly);
      const now = new Date().toISOString();
      setPosts((prev) =>
        prev.map((p) =>
          p.id === approvePost.id ? { ...p, posted: true, status: 'posted', posted_at: now } : p
        )
      );
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <div className="flex gap-2">
          ...
          <Tooltip>
            <TooltipTrigger asChild>
              <Button
                className="bg-[#6366F1] hover:bg-[#4F46E5] text-white rounded-full"
              ...
              <div className="flex items-center gap-2" onClick={(e) => e.stopPropagation()}>
                  ...
                  <>
                    <Tooltip>
                      <TooltipTrigger asChild>
                        <Button
                          size="sm"
                          variant="outline"
                          className="rounded-full border-[#6366F1] text-[#6366F1] hover:bg-[#6366F1]/10"
                    ...
                    <Tooltip>
                      <TooltipTrigger asChild>
                        <Button
                          size="sm"
                          className="rounded-full bg-[#6366F1] hover:bg-[#4F46E5]"
                          onClick={() => openPublishNow(post)}
                          disabled={!!actionLoading}
                        >
                ...
                {!post.media_url && !post.posted && (
                  <Tooltip>
                    ...
                    <TooltipContent side="bottom" sideOffset={6}>
                      {kieKeyPaid === true ? 'Generate image from post content' : 'Add and verify your KIE API key in Activity Settings'}
                    </TooltipContent>
                  </Tooltip>
                )}
                {!post.posted && (
                  <Tooltip>
                    ...
                    <TooltipContent side="bottom" sideOffset={6}>
                      {kieKeyPaid === true ? 'Generate video from post content (text-to-video)' : 'Add and verify your KIE API key in Activity Settings'}
                    </TooltipContent>
                  </Tooltip>
                )}
                <Tooltip>
        ...
        <DialogContent className="max-w-lg">
          ...
          <div className="space-y-4 py-4">
            ...
            <div className="space-y-3 rounded-xl bg-[#f8fafc] p-3">
              ...
              {kieKeyPaid !== true && (
                <p className="text-xs text-amber-600">Add and verify your KIE API key in Activity Settings to enable image and video generation.</p>
              )}
              <div className="flex items-center gap-2">
                <Checkbox
      ...
      <Dialog open={isEditDialogOpen} onOpenChange={setIsEditDialogOpen}>
        <DialogContent className="max-w-lg">
          ...
          <DialogFooter>
            ...
            <Button className="bg-[#6366F1] hover:bg-[#4F46E5]" onClick={handleUpdatePost}>
              Save Changes
            </Button>
          </DialogFooter>
        </DialogContent>
                ...
                <div className="bg-white rounded-lg shadow-sm overflow-hidden">
                  ...
                  <div className="px-4 py-3 flex items-start gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-[12px] text-[#00000099] mt-0.5">POSTORA</p>
                    </div>
                    ...
                    <>
                      ...
                      {!viewPost.media_url ? (
                        <Button
                          ...
                          disabled={!!actionLoading || kieKeyPaid !== true}
                          title={kieKeyPaid !== true ? 'Add and verify your KIE API key in Activity Settings' : undefined}
                        >
                      ...
                      <Button
                        ...
                        disabled={!!actionLoading || kieKeyPaid !== true}
                        title={kieKeyPaid !== true ? 'Add and verify your KIE API key in Activity Settings' : viewPost.video_url ? 'Regenerate video from post content' : 'Generate video from post content (text-to-video)'}
                      >
        ...
        <DialogContent className="max-w-md">
          ...
          <DialogFooter>
            ...
            {scheduleChoice === 'now' && approvePost?.status === 'pending' ? (
              <Button
                className="bg-[#6366F1] hover:bg-[#4F46E5]"
                onClick={() => approvePost && openPublishNow(approvePost, true)}
                disabled={!!actionLoading}
              >
      ...
      {/* Publish choice: with image/video or content only */}
      <Dialog open={!!publishChoicePost} onOpenChange={(open) => !open && setPublishChoicePost(null)}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle>How do you want to publish?</DialogTitle>
          </DialogHeader>
          <p className="text-sm text-[#6B7098]">
            This post has an image or video. You can publish with media or as text only.
          </p>
          <div className="flex gap-3 pt-2">
            <Button
              className="flex-1 bg-[#6366F1] hover:bg-[#4F46E5]"
              onClick={() => publishChoicePost && handlePublishChoice(false)}
              disabled={!!actionLoading}
            >
              {actionLoading === `publish:${publishChoicePost?.id}` ? (
                <Loader2 className="w-4 h-4 animate-spin mr-2" />
              ) : (
                <Image className="w-4 h-4 mr-2" />
              )}
            ...
            <Button
              variant="outline"
              className="flex-1"
              onClick={() => publishChoicePost && handlePublishChoice(true)}
              disabled={!!actionLoading}
            >
              Content only
            </Button>
