...
/** From comment_replies or reply_logs (same shape). */
type CommentReply = {
  id: string;
  post_id: string | null;
  post_uri: string | null;
  ...
  _source?: 'comment_replies' | 'reply_logs';
};
...
type ActivityItem = {
  ...
  myReply?: string;
  /** LinkedIn post URL (from engagement_logs or built from post_uri for replies) */
  permalink?: string | null;
...
const CommentsActivity = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      const replyLogsPromise = (async (): Promise<CommentReply[]> => {
        try {
          const res = await client
            .from('reply_logs')
            .select('id, post_id, post_uri, comment_urn, comment_text, reply_text, status, created_at')
            .eq('user_id', userId)
      ...
      let fromCommentReplies: CommentReply[] = (commentRepliesRes.data as CommentReply[]) || [];
      const fromReplyLogs: CommentReply[] = Array.isArray(replyLogsData) ? replyLogsData : [];

      fromCommentReplies = fromCommentReplies.map((r) => ({ ...r, _source: 'comment_replies' as const }));
      const replyLogsNormalized = fromReplyLogs.map((r) => ({ ...r, _source: 'reply_logs' as const }));

      const dedupeKey = (r: CommentReply) =>
        `${r.comment_urn ?? ''}|${r.reply_text ?? ''}|${new Date(r.created_at).getTime()}`;
      const seen = new Set<string>();
      const merged: CommentReply[] = [];
      for (const r of [...fromCommentReplies, ...replyLogsNormalized]) {
    ...
    const replyLogsChannel = client
      .channel('reply-logs')
      .on(
        'postgres_changes',
        { event: '*', schema: 'public', table: 'reply_logs', filter: `user_id=eq.${userId}` },
        fetchData
      )
      .subscribe();
    ...
    return () => {
      ...
      client.removeChannel(replyLogsChannel);
    };
  }, [user]);
  ...
  const activities: ActivityItem[] = [
    ...
    ...commentReplies.map((r) => {
      const postUri = r.post_uri ?? null;
      let permalink: string | undefined;
      if (postUri) {
        if (postUri.startsWith('urn:li:activity:')) {
          permalink = `https://www.linkedin.com/feed/update/${postUri}`;
        } else if (postUri.startsWith('urn:li:ugcPost:')) {
          const id = postUri.replace(/^urn:li:ugcPost:/i, '').trim();
          permalink = id ? `https://www.linkedin.com/feed/update/urn:li:activity:${id}` : undefined;
        } else if (postUri.startsWith('http://') || postUri.startsWith('https://')) {
          permalink = postUri;
        }
      }
      return {
        ...
        postContent: r.posts?.content ?? null,
        ...
        {filteredActivities.length === 0 ? (
          <div className="bg-white rounded-2xl p-12 card-shadow text-center">
            ...
            <p className="text-sm text-[#6B7098] mt-1">
              Likes, comments, and replies from your tools will appear here
            </p>
          </div>
        ) : (
          filteredActivities.map((item) => (
            <div
              key={item.id}
              className="bg-white rounded-2xl overflow-hidden card-shadow border border-[#6B7098]/5 cursor-pointer hover:border-[#6366F1]/30 hover:shadow-md transition-all"
              onClick={() => {
                setViewItem(item);
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {item.permalink && (
                      <a
                        ...
                        className="inline-flex items-center gap-1.5 mt-2 text-xs text-[#6366F1] hover:underline"
                      >
